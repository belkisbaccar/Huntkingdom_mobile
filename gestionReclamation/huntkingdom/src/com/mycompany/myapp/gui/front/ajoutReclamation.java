...
public class ajoutReclamation extends Form {
    ...
    public ajoutReclamation(Form previous, Utilisateur user) {
        current = this;
...
//reclamer.setBgImage(MyApplication.theme.getImage("b.jpg"));
    ...
    String  filePath = Capture.capturePhoto(Display.getInstance().getDisplayWidth(), -1);
        ...
        String url = "http://localhost/groupee/symfony2/web/upload.php";
cr.setUrl(url);
cr.setPost(true);
...
NetworkManager.getInstance().addToQueueAndWait(cr);
        ...
        btnValider.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                if ((tfDescription.getText().length() == 0) || (tfImage.getText().length() == 0) || (tfProd.getText().length() == 0)) {
                    Dialog.show("Alert", "SVP , Veulliez remplir les champs", new Command("OK"));
                } else {
                    try {

                        r.setDescription(tfDescription.getText());
                        r.setId_produit(Integer.parseInt(tfProd.getText()));
...
m.setMimeType(Message.MIME_HTML);
        ...
        addAll(tfDescription, tfImage, v1, tfProd, btnValider);
    }
